 ...
 ***************************************************/
if bedrockcounter>40    ///create new blocks
{    
    //show_message(bedrockcounter)
    
    for (i=0;i<ds_list_size(blockstosprite_prevlayer);i++)
    {
    ...
    for (i=0; i<kouchou.rw/40; i+=1)   ///create blocks by horizontal line
    {
        ...
        if makeok
        {
            ...
            switch typeofblock
            {
                ...
                case 3:
                ds_list_add(blockstosprite_prevlayer,spawn_gunpowder_block(kouchou.room_left_border_x+i*40,argument0,0))       ///gunnysnake doesn't make a block so this makes one for the initial position             
